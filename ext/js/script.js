...
$('.ar').on('click', function() {
  window.open(' https://gordion.github.io/magiword/', '_blank');
});
...
$(".dropdown").on('click', function(){
  ...
  $(this).toggleClass("opened");
    ...
    $('#transl').click(function (e){
      ...
      $('#add-dict').on('click', function(event){
        var selectedText = $.trim($(".textArea").val());
        var user = JSON.parse(localStorage.getItem('user'));
        var params = 'word=' + selectedText.replace(' ', '') + '&guid=' + user.guid;
        console.log(params)
          $.post('https://hybro.in.ua/roma.php?method=AddWord', params)
            .done(function(data){
              console.log(data);
              if (data.successful) {
                alert('Added');
